//! Integration tests for the S3 (MinIO) durable backend.
//!
//! Requirements:
//! - DragonflyDB on localhost:16379
//! - MinIO on localhost:19000 with bucket `corp-store-test`
//!
//! Run via:
//! ```sh
...
mod s3_tests {
    ...
    /// Build an S3Backend pointed at the local MinIO instance.
    fn minio_backend() -> S3Backend {
        ...
        S3Backend::new(client, "corp-store-test".into(), String::new())
    }

    /// Open a Redis connection to DragonflyDB on the integration port.
    ...
    /// Flush all KV keys for an entity (simulates data loss).
