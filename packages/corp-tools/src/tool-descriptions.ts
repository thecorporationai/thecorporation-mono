...
export function describeToolCall(name: string, args: Record<string, unknown>): string {
  ...
  const fmts: Record<string, string> = {
    ...
    create_valuation: 'Create {valuation_type} valuation effective {effective_date}',
    submit_valuation_for_approval: 'Submit valuation for board approval',
    approve_valuation: 'Approve valuation',
    start_equity_round: 'Start equity round "{name}"',
    add_security: 'Add {quantity} shares to {recipient_name} in round',
    issue_round: "Issue all securities and close the round",
    issue_equity: "Issue {shares} {grant_type} shares to {recipient_name}",
    transfer_shares: "Direct transfer {shares} shares (bypass governance)",
    ...
    send_notice: "Send notice for meeting",
    adjourn_meeting: "Adjourn meeting",
    cancel_meeting: "Cancel meeting",
    finalize_agenda_item: "Finalize agenda item to {status}",
    compute_resolution: "Compute resolution for agenda item",
    attach_resolution_document: "Attach document to resolution",
    written_consent: 'Create written consent: "{title}"',
    list_agenda_items: "List agenda items for meeting",
    list_votes: "List votes on agenda item",
