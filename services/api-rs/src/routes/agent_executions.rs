...
async fn authorize_execution(
    ...
    let actual_agent_id: Option<String> = conn
        .hget(&state_key, "agent_id")
        .await
    ...
    let actual_workspace_id: Option<String> = conn
        .hget(&state_key, "workspace_id")
        .await
        .map_err(|e| AppError::Internal(format!("redis hget: {e}")))?;
    let Some(actual_workspace_id) = actual_workspace_id else {
        return Err(AppError::Forbidden(
            "execution workspace missing".to_owned(),
        ));
    };
...
async fn get_execution_result(
    ...
    match json {
        ...
        None => Err(AppError::NotFound(format!(
            "result for {execution_id} not found"
        ))),
    }
}
